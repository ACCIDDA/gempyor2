...
class RHSFunction(Protocol):
    ...
    def __call__(
        ...
        state : NDArray[np.floating]
            Current state array of shape (n_states, n_subgroups).
...
class CoreSolver:
    ...
    - Supports Crank-Nicolson or predictor-corrector integration via
      pre-built operators.
    - Falls back to a direct assignment "Euler" mode when no operators are given.
    ...
    def __init__(
        ...
        ----------
        core : ModelCore
            The model core object managing state and time.
        operators : CoreOperators | None
            - For Crank-Nicolson: (L_op, R_op)
            - For predictor-corrector: (predictor, L_op, R_op)
            - If None, fallback is direct assignment (Euler-like).
        """
        self.core = core
    ...
    def step(self, rhs: NDArray[np.floating]) -> None:
        ...
        rhs : NDArray[np.floating]
            ...
            Semantics
            ---------
            - For CN / predictor–corrector:
              `rhs` is the quantity to which the linear diffusion/regularization
              operators are applied (fed into implicit_solve).
    ...
    def run(self, rhs_func: RHSFunction) -> None:
        """
        Run the simulation over the entire time_grid.
        ...
        rhs_func : RHSFunction
            Function of the form:
                ...
                rhs_func(t, state) -> rhs

            where
            - t : float
                Current simulation time.
            - state : NDArray[np.floating], shape (n_states, n_subgroups)
                Current state from the core.
            - rhs : NDArray[np.floating], shape (n_states, n_subgroups)
                Quantity to feed into `step`.

        Notes:
        -----
        - This iterates over all time points except the last (since the last
          point is the end of the final step).
        - The semantics of `rhs` are as in `step`: it is not necessarily
          a time derivative.
        """
